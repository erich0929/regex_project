...
template <class T>
DfaNode* ConcatNode<T>::clone () {
		return new ConcatNode (*this);
}

/* implement StarNode */
template <class T>
StarNode<T>::StarNode(T* token) {
		//this -> token = new T (*token);
}


template class ConcatNode <DfaNode>;
//template class StarNode <DfaNode>;
//template class StarNode <SingleWordNode>;
//template class StarNode <ConcatNode<T>>;
